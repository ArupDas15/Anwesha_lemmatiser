...
class document_linearization:
    ...
    def lem(self, word):
        ...
        for sentence_no, q_sent in enumerate(sentence_tokens):
            ...
            # Cheking if the word is present in the root words dictionary corpus.
            for w in words:
                if w == word:
                    return word
...
def read_train_data():
    ...
    for dirpath, dirnames, filenames in os.walk("Mini_Train_Dataset"):
...
read_train_data()
dl = document_linearization()
for i in range(0, len(dids)):
    print(file_paths[i].replace("Mini_Train_Dataset", "Lemmatization"))
    filewrite = open(file=file_paths[i].replace("Mini_Train_Dataset", "Lemmatization"),
                     mode="w",
                     encoding='utf-8',
                     errors='ignore')
    filewrite.write(' '.join([str(elem) for elem in dl.pre_process(docs[i])]))
